...
class Partial_Loading_Dataset(TrajectoryDataset):
    def __init__(
        ...
        for i, traj_dir in enumerate(tqdm(self.traj_dirs)):
            ...
            feature_path = traj_dir / self.feature_name

            self.features.append(torch.load(feature_path).int())
    ...
    def __getitem__(self, idx):
        ...
        if i < len(self.trajs):
            ...
            for cam in range(2):
                ...
                for index in range(start, end):
                    ...
                    boxes = self.features[i][cam][index].int()
                    mask = np.zeros(nparr.shape)
                    for box in boxes:
                        mask[box[1] : box[3], box[0] : box[2], :] = 1
                    processed = np.where(mask, nparr, mask)
                    processed = preprocess_img_no_crop(
                        processed,
                        resize=self.cams_resize[cam],
                        device=self.device,
        ...
        cams_features = read_img_from_hdf5(
        ...
        return cam_0, cam_1, f0, f1, act, mask
